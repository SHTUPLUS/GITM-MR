...
class UniterEncoder(nn.Module):
    def __init__(self, config, use_layer):
        super().__init__()
    ...
    def forward(self, input_, attention_mask,
                output_all_encoded_layers=True,remain_att=False):
        all_encoder_layers = []
        all_layer_atts=[]
        ...
        for layer_module in self.layer:
            ...
            if remain_att:
                all_layer_atts.append(attention_probs)
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        return all_encoder_layers,all_layer_atts


class UniterModel(UniterPreTrainedModel):
    """ Modification for Joint Vision-Language Encoding
    """
    def __init__(self, config, img_dim, use_layer):
        super().__init__(config)
        self.embeddings = UniterTextEmbeddings(config)
        self.img_embeddings = UniterImageEmbeddings(config, img_dim)
        self.encoder = UniterEncoder(config,use_layer)
    ...
    def _compute_img_txt_embeddings(self, input_ids, position_ids,
        ...
        embedding_output = torch.gather(torch.cat([txt_emb, img_emb], dim=1),
                                        dim=1, index=gather_index)#img_len+txt_len dismensions are useful
    ...
    def forward(self, input_ids, position_ids,
                ...
                txt_type_ids=None, img_type_ids=None,remain_att=False):
        ...
        encoded_layers,all_layer_atts = self.encoder(
            embedding_output, extended_attention_mask,
            output_all_encoded_layers=output_all_encoded_layers,remain_att=remain_att)
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
        return encoded_layers,all_layer_atts
